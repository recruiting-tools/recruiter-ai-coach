 ...
 * @param {Goal[]} goals - Active interview goals
 * @param {object} [opts]
 * @param {boolean} [opts.noThrottle=false] - Disable 20s throttle
 * @returns {Promise<string|null>}
 */
async function generateGoalsAwareHint(sessionId, segment, goals, { noThrottle = false } = {}) {
  ...
  const systemPrompt = GOALS_AWARE_SYSTEM_PROMPT + goalsSection;

  const userMessage = `${prepSection}
Транскрипция последних реплик:
${recentText}

Новая реплика: ${segment.speaker}: ${segment.text}

Нужна ли рекрутеру подсказка прямо сейчас? Учти активные цели интервью. Если кандидат упоминает технологию — предложи умную фразу. Если нет важного — пустую строку.`;
