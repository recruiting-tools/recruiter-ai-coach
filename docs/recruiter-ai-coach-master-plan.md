...
Chrome Extension
  ...
  └── offscreen.js         → MediaRecorder → WebSocket → backend
  └── content.js           → overlay UI поверх Meet (транскрипт, подсказки, keywords, goals)
...
Node.js Backend (Express + WS)
  └── server.js            → роутер, WS handler
  └── deepgram-stream.js   → Deepgram nova-2, потоковая транскрипция
  └── claude.js            → GPT-4o (подсказки, prep kit, keyword explain)
  └── interview-goals-engine.js → rule-based оценка целей
  └── interview-config-db.js    → SQLite: interviews, candidates, jobs, sessions
  └── routes/interview-config-routes.js → REST CRUD
...
- [ ] Web страница со стримингом подсказок (для второго экрана / телефона)
- [ ] Speaker separation: Deepgram diarize vs mic+tab два потока
- [ ] Воск: нужна ли немецкая/английская модель или только ru?
